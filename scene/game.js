...
function skip()
{
  ...
  for (let i = 0; i < 4; i++)
  {
    for (let j = 0; j < 4; j++)
    {
      let max_y_private = 99;
      for (let k = y + i; k < 18; k++)
      {
        if (blockData[k][j] != 0)
        {
          max_y_private = k;
      ...
      if (max_y_private == 99) max_y_private = 13 + i;
      if (max_y > max_y_private) max_y = max_y_private;
      //console.log("private y = " + max_y_private);
    }
  }
  y = max_y + 1;
  control = false;
